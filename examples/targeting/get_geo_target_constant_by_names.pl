...
use
  Google::Ads::GoogleAds::V1::Services::GeoTargetConstantService::LocationNames;
...
sub get_geo_target_constant_by_names {
  my ($client, $location_names, $locale, $country_code) = @_;

  my $suggest_response = $client->GeoTargetConstantService()->suggest({
      locale      => $locale,
...
# Call the example.
get_geo_target_constant_by_names($client, $location_names, $locale,
  $country_code);
