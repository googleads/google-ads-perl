...
use Google::Ads::GoogleAds::V1::Enums::MediaTypeEnum qw(IMAGE);
use Google::Ads::GoogleAds::V1::Services::MediaFileService::MediaFileOperation;
...
sub upload_image {
  ...
  my $image_url = "https://goo.gl/3b9Wfh";

  # Create a media image.
  my $media_image = Google::Ads::GoogleAds::V1::Resources::MediaImage->new({
      data => get_base64_data_from_url($image_url)});

  # Create a media file.
  my $media_file = Google::Ads::GoogleAds::V1::Resources::MediaFile->new({
    name      => "Ad Images",
    type      => IMAGE,
    sourceUrl => $image_url,
    image     => $media_image
  ...
  my $media_file_operation =
    Google::Ads::GoogleAds::V1::Services::MediaFileService::MediaFileOperation
    ->new({
      create => $media_file
    });

  # Add the media file.
  my $media_file_response = $client->MediaFileService()->mutate({
...
# Call the example.
upload_image($client, $customer_id =~ s/-//gr);
