...
sub generate_forecast_metrics {
  my ($client, $customer_id, $keyword_plan_id) = @_;

  my $forecast_metrics_response =
    $client->KeywordPlanService()->generate_forecast_metrics(
    Google::Ads::GoogleAds::V1::Utils::ResourceNames::keyword_plan(
...
# Call the example.
generate_forecast_metrics($client, $customer_id =~ s/-//gr, $keyword_plan_id);
