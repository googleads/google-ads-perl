...
use
  Google::Ads::GoogleAds::V1::Services::BillingSetupService::BillingSetupOperation;
...
sub remove_billing_setup {
  ...
  my $billing_setup_response = $client->BillingSetupService->mutate({
...
# Call the example.
remove_billing_setup($client, $customer_id =~ s/-//gr, $billing_setup_id);
