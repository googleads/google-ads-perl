...
use Google::Ads::GoogleAds::V1::Enums::PolicyApprovalStatusEnum qw(DISAPPROVED);
use
...
sub get_all_disapproved_ads {
  ...
  while ($iterator->has_next) {
    ...
    next if $policy_summary->{approvalStatus} ne DISAPPROVED;
...
# Call the example.
get_all_disapproved_ads($client, $customer_id =~ s/-//gr, $campaign_id);
