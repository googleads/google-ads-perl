...
sub create_complete_campaign_adwords_api_only {
  my ($adwords_client) = shift;

  my $campaign_budget = create_campaign_budget($adwords_client);
  my $campaign =
    create_campaign($adwords_client, $campaign_budget->get_budgetId());
...
sub create_campaign_budget {
  ...
  my $created_budget = $adwords_client->BudgetService()
    ->mutate({operations => [$budget_operation]})->get_value();

  printf
    "Budget with ID %d and name '%s' was created.\n",
    $created_budget->get_budgetId(),
    $created_budget->get_name();
...
This code example is the first in a series of code examples that show how to create
...
This code example represents the initial state, where the AdWords API is used to
create a campaign budget, a search campaign, ad groups, keywords and expanded text ads.
None of the functionality has yet been migrated to the Google Ads API.
