...
#
# This example removes a FeedItemAttributeValue of a FeedItem in a flights
# Feed. To create a flights feed, run the add_flights_feed.pl example.
# This example is specific to feeds of type DYNAMIC_FLIGHT. The attribute
# you are removing must be present on the feed.
...
sub remove_feed_item_attribute_value {
  ...
  my $feed_item =
    ...
    $feed_attributes, $feed_item_resource_name, uc($flight_placeholder_field));
...
sub remove_attribute_value_from_feed_item {
  my ($api_client, $customer_id,
    $feed_attributes, $feed_item_resource_name, $flight_placeholder_field)
    = @_;

  # Get the ID of the FeedAttribute for the placeholder field.
  my $attribute_id = $feed_attributes->{$flight_placeholder_field}{id};
...
# Call the example.
remove_feed_item_attribute_value($api_client, $customer_id =~ s/-//gr,
  $feed_id, $feed_item_id, $flight_placeholder_field);
...
This example removes a FeedItemAttributeValue of a FeedItem in a flights Feed.
To create a flights feed, run the add_flights_feed.pl example. This example is
specific to feeds of type DYNAMIC_FLIGHT. The attribute you are removing must
