...
use Google::Ads::GoogleAds::V1::Enums::ManagerLinkStatusEnum qw(PENDING ACTIVE);
use
  Google::Ads::GoogleAds::V1::Services::CustomerClientLinkService::CustomerClientLinkOperation;
use
  Google::Ads::GoogleAds::V1::Services::CustomerManagerLinkService::CustomerManagerLinkOperation;
...
sub link_manager_to_client {
  ...
  my $client_link_response = $client->CustomerClientLinkService()->mutate({
  ...
  my $search_response = $client->GoogleAdsService()->search({
    customerId => $manager_customer_id,
    query      => $search_query
  });
  ...
  my $manager_link_resource_name =
    Google::Ads::GoogleAds::V1::Utils::ResourceNames::customer_manager_link(
  ...
  # as the client.
  $client->set_login_customer_id($client_customer_id);
  ...
  my $manager_link_response = $client->CustomerManagerLinkService()->mutate({
