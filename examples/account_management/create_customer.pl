...
sub create_customer {
  my ($client, $manager_customer_id) = @_;

  # Initialize a customer to be created.
  my $customer = Google::Ads::GoogleAds::V1::Resources::Customer->new({
  ...
  my $create_customer_client_response =
    $client->CustomerService()->create_customer_client({
...
# Call the example.
create_customer($client, $manager_customer_id =~ s/-//gr);
