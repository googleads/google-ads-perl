...
sub get_account_information {
  ...
  my $resource_name =
    Google::Ads::GoogleAds::V1::Utils::ResourceNames::customer($customer_id);

  my $customer = $client->CustomerService()->get($resource_name);
...
# Call the example.
get_account_information($client, $customer_id =~ s/-//gr);
