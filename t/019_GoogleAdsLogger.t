...

{
  ...
  sub test_logging {
    my ($json_request, $json_response) = @_;
    GoogleAdsLogger::Test::Base::test_logging($json_request, $json_response);
  }
}

{
  ...
  sub test_logging {
    my ($json_request, $json_response) = @_;
    my $http_request  = __wrap_http_request($json_request);
    my $http_response = __wrap_http_response($json_response);
  ...
  sub __wrap_http_request {
    ...
    return HTTP::Request->new(
      $json_request->{method},  $json_request->{url},
      $json_request->{headers}, encode_json($json_request->{content}));
  }

  # The private method to wrap a HTTP response from a JSON object.
  sub __wrap_http_response {
    my $json_response = shift;
    return HTTP::Response->new(
      $json_response->{code},    $json_response->{url},
      $json_response->{headers}, encode_json($json_response->{content}));
  }
}
