...
$path_template = "v2/customers/{+customersId}/adGroups:mutate";
is(
  expand_path_template($path_template, $customer_id),
  "v2/customers/12345/adGroups:mutate",
  "Test expand_path_template(): expand with {+customersId}."
);
$path_template = "v2/googleAdsFields:search";
is(expand_path_template($path_template),
  "v2/googleAdsFields:search", "Test expand_path_template(): no expand");

# Tests the copy_from() method.
my $original_campaign = Google::Ads::GoogleAds::V2::Resources::Campaign->new({
    name                   => "Interplanetary Cruise",
    advertisingChannelType => "SEARCH",
    status                 => "PAUSED",
    biddingStrategy        => "MANUAL_CPC",
