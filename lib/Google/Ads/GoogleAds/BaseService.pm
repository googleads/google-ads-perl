...
sub call {
  ...
  if ($path_params) {
    ref $path_params
      ? $request_path =
      expand_path_template($request_path, $path_params->{resourceName})
      : $request_path = expand_path_template($request_path, $path_params);
  }

  # POST: If the 'customer_id' in the $request_body argument is present, use
  # it to expand the {+customerId} expression in the request path.
  $request_path =
    expand_path_template($request_path, $request_body->{customerId})
    if $request_body and $request_body->{customerId};

  # Generate the endpoint from the API server address and the request path.
  my $endpoint = $api_client->get_service_address() . $request_path;
  ...
  my $http_request =
    $auth_handler->prepare_request($http_method, $endpoint, $http_headers,
    $request_content);
  ...
  # Log the one-line summary and traffic detail.
  ...
  if ($http_response->is_success) {
    ...
    bless $json_response, $response_type;
