...
# Expands a path template by replacing the parameters in braces with
# the given arguments.
sub expand_path_template {
  ...
  my @matches = ($path_template =~ /{\w+}/g);

  for (my $i = 0 ; $i < @matches ; $i++) {
    $path_template =~ s/$matches[$i]/$args[$i]/ if $args[$i];
  }
...
'v1/customers/{+customerId}/adGroups:mutate'.

=item *

I<args>: the args in array used to expand the template.
