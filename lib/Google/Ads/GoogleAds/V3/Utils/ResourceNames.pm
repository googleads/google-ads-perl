...
package Google::Ads::GoogleAds::V3::Utils::ResourceNames;
...
sub ad_group_ad_asset_view {
  my ($customer_id, $ad_group_id, $ad_id, $asset_id, $asset_field_type) = @_;

  my $path_template =
'customers/{customer_id}/adGroupAdAssets/{ad_group_id}~{ad_id}~{asset_id}~{asset_field_type}';

  return expand_path_template($path_template,
    [$customer_id, $ad_group_id, $ad_id, $asset_id, $asset_field_type]);
}
...
'customers/{customer_id}/keywordPlanNegativeKeywords/{kp_negative_keyword_id}';
  ...
  return expand_path_template($path_template,
    [$customer_id, $kp_negative_keyword_id]);
}
